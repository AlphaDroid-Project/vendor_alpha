...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^alpha_") ; then
        ALPHA_BUILD=$(echo -n $1 | sed -e 's/^alpha_//g')
    else
        ALPHA_BUILD=
    fi
    export ALPHA_BUILD
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/AlphaDroid-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.alpha.device | grep -q "$ALPHA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $ALPHA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.alpha.device | grep -q "$ALPHA_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.alpha.device | grep -q "$ALPHA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $ALPHA_FIXUP_COMMON_OUT ]; then
...
function build_kernel() {
    ...
    if [[ "${SKIP_KERNEL_SYNC}" != "true" && "${SKIP_KERNEL_SYNC}" != "1" ]]; then
        ...
        local repo_init_args=("-b" "${alpha_branch}")
        if [ -n "${REPO_VERSION}" ]; then
            repo_init_args+=("--repo-rev" "${REPO_VERSION}")
        fi

        yes | repo init -u https://github.com/alphadroid-project/${target_kernel_manifest}.git ${repo_init_args[@]} || [ $? -eq 141 ]
        if [ $? -ne 0 ]; then
